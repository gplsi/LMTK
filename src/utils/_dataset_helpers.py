    ...
    for root, dirs, files in os.walk(path):
        ...
        if source != os.path.basename(path):  # Exclude the root directory itself
            if (extension is not None):
                data_files = [os.path.join(root, file) for file in files if file.endswith(f'.{extension}')]  # Filter by extension
            else:
                data_files = [os.path.join(root, file) for file in files if file.split('.')[-1] in SUPPORTED_EXTENSIONS]
