"""
Distributed training strategies for instruction fine-tuning implemented with Lightning Fabric.
"""
...
from src.tasks.instruction.fabric.base import FabricTrainerBase
from src.tasks.instruction.fabric.wrappers.fsdp_config import resolve_fsdp_config
...
class FSDP(FabricTrainerBase):
    """
    Class to set up the Fully Sharded Data Parallel (FSDP) strategy for instruction fine-tuning.
...
class DeepSpeed(FabricTrainerBase):
    """
    Class to set up the DeepSpeed strategy for instruction fine-tuning.
    ...
    def _setup_strategy(self):
        ...
        if self.devices > 1:
            ...
            strategy = DeepSpeedStrategy(
                zero_stage=self.config.get("zero_stage", 2),  # e.g. 2 or 3
                offload_optimizer=self.config.get("offload_optimizer", False),  # True/False
                offload_parameters=self.config.get("offload_parameters", False),  # True/False
...
class DistributedDataParallel(FabricTrainerBase):
    """
    Class to set up the Distributed Data Parallel (DDP) strategy for instruction fine-tuning.
...
class DataParallel(FabricTrainerBase):
    """
    Class to set up the Data Parallel (DP) strategy for instruction fine-tuning.
