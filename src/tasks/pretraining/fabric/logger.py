...
def step_csv_logger(*args: Any, cls: Type[T] = CSVLogger, **kwargs: Any) -> T:
    ...
    def merge_by(dicts: List[Dict], key: str) -> List[Dict]:
        ...
        # Group dictionaries by their common key using defaultdict.
        out = defaultdict(dict)
        for d in dicts:
            if key in d:
                out[d[key]].update(d)
        # Return the merged dictionaries sorted by the key.
    ...
    def save(self) -> None:
        ...
        # If there are no metrics recorded, exit early.
        if not self.metrics:
            return
        
        # Merge metrics entries using the 'step' key.
        metrics = merge_by(self.metrics, "step")
        # Determine CSV header columns from the merged metrics.
        keys = sorted({k for m in metrics for k in m})
        # Open the CSV file for writing using the experiment's file system.
        with self._fs.open(self.metrics_file_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=keys)
            writer.writeheader()
            writer.writerows(metrics)

    # Attach the custom save method to the experiment using MethodType.
