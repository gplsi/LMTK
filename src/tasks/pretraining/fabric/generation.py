...
class FabricGeneration(L.LightningModule):
    ...
    def training_step(self, batch: Tuple[torch.Tensor, ...], *args) -> dict:
        ...
        """
        # Forward pass: Compute outputs and loss for the input batch.
        outputs = self.model(
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, ...], *args) -> dict:
        ...
        """
        # Forward pass: Compute outputs and loss for the validation batch.
        outputs = self.model(
            input_ids=batch['input_ids'],
            attention_mask=batch['attention_mask'],
            labels=batch['labels'],
    ...
    def test_step(self, batch: Tuple[torch.Tensor, ...], *args) -> dict:
        ...
        """
        # Forward pass: Compute outputs and loss for the test batch.
        outputs = self.model(
            batch['input_ids'],
