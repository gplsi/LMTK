...
- EOS resolution in training is offline-safe: it does not fetch tokenizer data from the Hub. If `packing.eos_token_id` is not provided, training resolves EOS from `dataset.sources[*].eos_token_id` or from dataset sidecar metadata (`lmtk_tokenization_meta.json`). Existing tokenized datasets can be patched with `scripts/backfill_tokenized_dataset_metadata.py`.
